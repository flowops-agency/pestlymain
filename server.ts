...
app.post('/api/livekit-token', async (req, res) => {
  try {
    ...
    const roomName = `room-de-${Math.random().toString(36).substring(7)}`;
    ...
    // Agent explizit dispatchen
    try {
      const dispatchClient = new AgentDispatchClient(livekitUrl!, apiKey!, apiSecret!);
      await dispatchClient.createDispatch(roomName, "DE-Web-AI");
      console.log(`Agent DE-Web-AI in Raum ${roomName} dispatched.`);
    } catch (error) {
      console.error('Agent Dispatch fehlgeschlagen:', error);
    }
...
app.post('/api/livekit-token-us', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`Agent US-Web-AI in Raum ${roomName} dispatched.`);
    } catch (error) {
      console.error('Agent Dispatch fehlgeschlagen:', error);
    }
